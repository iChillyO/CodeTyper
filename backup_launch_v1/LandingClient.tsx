...
import {
    Terminal, Play, Code, Zap, Video, Check, X, ArrowRight,
    Layers, Monitor, ShieldCheck, Sparkles, Github, Rocket, Users,
    Eye, Settings, Download, Film, Flame, Target, CreditCard
} from "lucide-react";
    ...
    return (
        <>
            ...
            <section id="pricing" className="py-40 px-6 border-t border-white/5">
                <div className="max-w-7xl mx-auto">
                    <div className="text-center mb-24">
                        <h2 className="text-4xl md:text-5xl font-black mb-6 tracking-tight flex items-center justify-center gap-4"><CreditCard className="w-10 h-10 text-green-500" /> Simple Pricing</h2>
                        <p className="text-slate-500 font-medium">Ready to scale your content? Choose your plan.</p>
                    </div>
                    ...
                    <div className="grid lg:grid-cols-3 gap-8">
                        <PricingCard
                            tier="Free"
                            price="0"
                            features={["5 renders/month", "720p export quality", "Watermark included", "Standard themes"]}
                        />
                        <PricingCard
                            tier="Creator"
                            price="9"
                            popular={true}
                            features={["100 renders/month", "1080p high quality", "No watermark", "All premium themes", "Personal share links"]}
                        />
                        <PricingCard
                            tier="Pro"
                            price="19"
                            features={["500 renders/month", "4K Ultra HD quality", "Priority rendering", "Everything in Creator", "Priority support"]}
                        />
                    </div>
                </div>
            </section>
...
function PricingCard({ tier, price, features, popular = false }: { tier: string, price: string, features: string[], popular?: boolean }) {
    return (
        <div className={`p-10 rounded-3xl border ${popular ? 'border-electric-blue bg-electric-blue/5 shadow-[0_0_40px_rgba(59,130,246,0.1)]' : 'border-white/5 bg-white/[0.02]'} flex flex-col items-center text-center relative overflow-hidden group`}>
            {popular && (
                <div className="absolute top-0 right-0 bg-electric-blue text-white text-[10px] font-black py-1 px-4 rounded-bl-xl uppercase tracking-widest shadow-xl">Most Popular</div>
            )}
            <h3 className="text-xl font-bold text-slate-400 mb-4 uppercase tracking-[.25em]">{tier}</h3>
            <div className="flex items-baseline gap-1 mb-10">
                <span className="text-2xl font-bold opacity-40">$</span>
                <span className={`text-6xl font-black ${popular ? 'text-electric-blue' : 'text-white'}`}>{price}</span>
                <span className="text-slate-500 font-bold ml-1">/mo</span>
            </div>
            <ul className="w-full space-y-4 text-left mb-12 border-t border-white/5 pt-8">
                {features.map((f, i) => (
                    <li key={i} className="flex items-center gap-3 text-sm font-medium text-slate-300">
                        <Check className={`w-4 h-4 ${popular ? 'text-electric-blue' : 'text-slate-500'}`} />
                        {f}
                    </li>
                ))}
            </ul>
            <Link href="/create" className="w-full">
                <button className={`w-full py-4 rounded-xl font-black text-sm uppercase tracking-widest transition-all ${popular ? 'bg-electric-blue text-white shadow-lg hover:scale-[1.02]' : 'bg-white/10 hover:bg-white/20'}`}>
                    Get Started
                </button>
            </Link>
        </div>
